...
class EditObjectInputs:
    ...
    def _create_point_inputs(self, point):
        ...
        self.parent.scroll_layout.addWidget(QLabel("Coordenadas (X, Y):"))
    ...
    def _create_line_inputs(self, line):
        ...
        self.parent.scroll_layout.addWidget(QLabel("Ponto 1 (X1, Y1):"))
        ...
        self.parent.scroll_layout.addWidget(QLabel("Ponto 2 (X2, Y2):"))
    ...
    def _create_bspline_inputs(self, bspline):
        ...
        self.num_bspline_points_input.setMinimum(4)  # B-Spline precisa de pelo menos 4 pontos para cúbica
    ...
    def _generate_bspline_fields(self):
        """Gera dinamicamente os campos de entrada para os pontos de controle da B-Spline."""
    ...
    def get_point_coordinates(self):
        if hasattr(self, 'x_input') and hasattr(self, 'y_input'):
            ...
            if self._is_valid_number(x_str) and self._is_valid_number(y_str):
    ...
    def get_line_coordinates(self):
        if all(hasattr(self, attr) for attr in ['x1_input', 'y1_input', 'x2_input', 'y2_input']):
            ...
            if all(self._is_valid_number(s) for s in [x1_str, y1_str, x2_str, y2_str]):
    ...
    def get_bezier_coordinates(self): # Fixed typo
        if hasattr(self, 'control_point_inputs'):
            ...
            for x_input, y_input in self.control_point_inputs:
                ...
                y_str = y_input.text().strip()
                if self._is_valid_number(x_str) and self._is_valid_number(y_str):
                    control_points.append((int(x_str), int(y_str)))
                else:
                    valid = False
                    break
    ...
    def get_bspline_coordinates(self):
        if hasattr(self, 'bspline_point_inputs'):
            control_points = []
            valid = True
            for x_input, y_input in self.bspline_point_inputs:
                x_str = x_input.text().strip()
                y_str = y_input.text().strip()
                if self._is_valid_number(x_str) and self._is_valid_number(y_str):
                    control_points.append((int(x_str), int(y_str)))
